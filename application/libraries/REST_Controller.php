...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
       ...
       if (version_compare(PHP_VERSION, '8.0.0', '<')) {
        ...
        $ContentLanguage = $this->input->get_request_header("Content-Language",TRUE);
        ...
        // Skip token test for $config['auth_override_class_method']['class'['method'] = 'none'
        if ($this->config->item('rest_enable_token') && $this->auth_override !== TRUE)
        {
    ...
    /**
     * See if the user has provided an API token
     *
     * @access protected
     * @return bool
     */
    protected function _detect_api_token()
    {
        ...
        $api_key_variable = $this->config->item('rest_token_name');
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        {
            if (!($row = $this->rest->db->where($this->config->item('rest_token_column'), $key)->get($this->config->item('rest_token_table'))->row()))
            {
                return FALSE;
            }

            $this->rest->key = $row->{$this->config->item('rest_token_column')};
    ...
    protected function _check_access()
    {
        ...
        return $this->rest->db
            ->where('key', $this->config->item('rest_key_name'))
